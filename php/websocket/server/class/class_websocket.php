...
class class_websocket {
    ...
    function run() {
        ...
        while (true) {
            ...
            socket_select($this->cycle, $write, $except, null);//获取$this->cycle数组中活动的socket，并且把不活跃的从$this->cycle数组中删除,具体的看文档。。


            // 回调 全部
            empty($this->function['all']) || call_user_func_array($this->function['all'], array($this->cycle, $this)); //$this->function['all']不为空，则执行call_user_func_array()函数
