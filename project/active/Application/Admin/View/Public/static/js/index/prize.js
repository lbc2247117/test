var
        GET_DATA_API = 'getPrizeData',
        GET_PRIZE = 'huoPrize',
        EDIT_API = 'getPrizeAction',
        CNT = new Vue({
            el: '#page-wrapper',
            data: {
                clickCount: 0,
            ...
            methods: {
                ...
                getData: function (like, type, sort, page, keyword, asc, cb) {
                    ...
                    $.post(GET_DATA_API, _dt, function (rst, status) {
                        ...
                        if (rst.status == '1') {
                            ...
                            CNT.prizeCount = rst.data.prizeCount;
                            CNT.getPrize = rst.data.getPrize;
                ...
                getPrizeFn: function () {
                    $.post(GET_PRIZE, {}, function (rst) {
                        rst = JSON.parse(rst);
                        if (rst.status == 1) {
                            for (var i = 0; i < rst.data.info.length; i++) {
                                CNT.prizeType.push(rst.data.info[i]);
                            }
                        }
                    });
                },
                ...
                showPrize: function () {
                    window.location.href = 'huoprize.html';
                },
                searchFn: function () {
                    this.curPage == 1;
                    this.getData();
