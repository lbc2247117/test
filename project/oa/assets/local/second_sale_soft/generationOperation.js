...
var GenerationOperationListViewModel = new function () {
    ...
    self_.listGenerationOperation = function (data) {
        ycoa.ajaxLoadGet("/api/second_sale_soft/generationOperation.php", data, function (results) {
            ...
            $.each(results.list, function (index, generationOperation) {
                ...
                generationOperation.dele = ycoa.SESSION.PERMIT.hasPagePermitButton('3060402');
                generationOperation.edit = ycoa.SESSION.PERMIT.hasPagePermitButton('3060403');
                generationOperation.show = ycoa.SESSION.PERMIT.hasPagePermitButton('3060404');
    ...
    self_.delGenerationOperation = function (generationOperation) {
        ycoa.UI.messageBox.confirm('确认删除？', function (del) {
            if (del) {
                ...
                ycoa.ajaxLoadPost("/api/second_sale_soft/generationOperation.php", JSON.stringify(generationOperation), function (result) {
                    if (result.code == 0) {
    ...
    self_.doEditSubmit = function (generationOperation) {
        ...
        ycoa.ajaxLoadPost("/api/second_sale_soft/generationOperation.php", data, function (result) {
            if (result.code == 0) {
                ycoa.UI.toast.success(result.msg);
                reLoadData({action: 1});
            } else {
...
$(function () {
    ...
    $("#add_fillarrears_form #qq").keypress(function (e) {
        if (e.keyCode == 13) {
            if ($(this).val()) {
                $.get(ycoa.getNoCacheUrl("/api/second_sale_soft/generationOperation.php"), {action: 5, qq: $(this).val()}, function (res) {
                    if (res.length == 1) {
                        var data = res[0];
                        $("#add_fillarrears_form input").each(function () {
                            var el = $(this);
    ...
    $("#btn_toexcel_primary").click(function () {
        ...
        if (start_time || end_time) {
            window.location.href = "/api/second_sale_soft/generationOperation.php?action=10&start_time=" + start_time + "&end_time=" + end_time;
        }
    });
    ...
    $.get(ycoa.getNoCacheUrl("/api/second_sale_soft/customer.php"), {action: 2, type: 5}, function (res) {
    ...
    $.get(ycoa.getNoCacheUrl("/api/second_sale_soft/platform.php"), {action: 2}, function (res) {
