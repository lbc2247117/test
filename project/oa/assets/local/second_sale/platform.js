var rception_array, customer_array;
var PlatformListViewModel = new function () {
    var self_ = this;
    self_.list = ko.observable("list");
    self_.platformList = ko.observableArray([]);
    self_.listPlatform = function (data) {
        ycoa.ajaxLoadGet("/api/second_sale/platform.php", data, function (results) {
            self_.platformList.removeAll();
            $.each(results.list, function (index, platform) {
                platform.isTeach_text = platform.isTeach === 1 ? "√" : "";
                platform.dele = ycoa.SESSION.PERMIT.hasPagePermitButton("3050102");
                platform.edit = ycoa.SESSION.PERMIT.hasPagePermitButton("3050103");
                platform.show = ycoa.SESSION.PERMIT.hasPagePermitButton("3050104");
                platform.te = ycoa.SESSION.PERMIT.hasPagePermitButton('3050106') && platform.isTe === 1;
    ...
    self_.delPlatform = function (platform) {
        ycoa.UI.messageBox.confirm('确认删除？', function (del) {
            if (del) {
                platform.action = 2;
                ycoa.ajaxLoadPost("/api/second_sale/platform.php", JSON.stringify(platform), function (result) {
                    if (result.code == 0) {
    ...
    self_.doEditSubmit = function (platform) {
        ...
        ycoa.ajaxLoadPost("/api/second_sale/platform.php", data, function (result) {
            if (result.code == 0) {
                ycoa.UI.toast.success(result.msg);
                reLoadData({action: 1});
            } else {
...
$(function () {
    ...
    $("#batchDelete").click(function(){
        ...
        if(id_ary.length==0){
            ...
            $.ajax({
                ...
                url:'../../api/second_sale/platform.php',
    ...
    $('#btn_importexcel_primary').click(function () {
        $('#importexcel_form').ajaxSubmit({
            type: 'post',
            url: "../../api/second_sale/platform.php?upload=1",
            success: function (result) {
                if (result.code == 0) {
    ...
    $("#dataTable .checkbox").live("click", function () {
        countMoney(5);
    });
    ...
    $("#add_fillarrears_form #qq").keypress(function (e) {
        if (e.keyCode == 13) {
            if ($(this).val()) {
                $.get(ycoa.getNoCacheUrl("/api/second_sale/platform.php"), {action: 5, qq: $(this).val()}, function (res) {
                    if (res.length == 1) {
                        var data = res[0];
                        $("#add_fillarrears_form input").each(function () {
                            var el = $(this);
    ...
    $("#btn_toexcel_primary").click(function () {
        ...
        if (start_time || end_time) {
            window.location.href = "/api/second_sale/platform.php?action=10&start_time=" + start_time + "&end_time=" + end_time;
        }
    });
...
//            showSeconds: false,
    ...
    $.get(ycoa.getNoCacheUrl("/api/second_sale/customer.php"), {action: 2, type: 2}, function (res) {
        $("#add_platform_form #customer").autoEditSelecter(res, function (d) {
            $("#add_platform_form #customer_id").val(d.id);
        });
        customer_array = res;
    });
    $("#show_ranking").click(function () {
        ycoa.ajaxLoadGet("/api/second_sale/platform.php", {action: 11, time_unit: 1}, function (result) {
            var html = "<ul>";
    ...
    $(".auto_tab_main .auto_tab_title li").click(function () {
        ...
        if (!self.hasClass("select")) {
            if ((data - parseInt($(".auto_tab_main").attr("t"))) > 500) {
                var v = self.attr("var");
                ycoa.ajaxLoadGet("/api/second_sale/platform.php", {action: 11, time_unit: v}, function (result) {
                    var html = "<ul>";
                    $.each(result, function (index, d) {
                        var clas = "num";
                        if (index == 0) {
