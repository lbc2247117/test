...
var DecorationListViewModel = new function () {
    ...
    self_.listDecoration = function (data) {
        ycoa.ajaxLoadGet("/api/second_sale/decoration.php", data, function (results) {
            ...
            $.each(results.list, function (index, decoration) {
                ...
                decoration.dele = ycoa.SESSION.PERMIT.hasPagePermitButton('3050302');
                decoration.edit = ycoa.SESSION.PERMIT.hasPagePermitButton('3050302');
                decoration.show = ycoa.SESSION.PERMIT.hasPagePermitButton('3050302');
    ...
    self_.delDecoration = function (decoration) {
        ycoa.UI.messageBox.confirm('确认删除？', function (del) {
            if (del) {
                decoration.action = 2;
                ycoa.ajaxLoadPost("/api/second_sale/decoration.php", JSON.stringify(decoration), function (result) {
                    if (result.code == 0) {
    ...
    self_.doEditSubmit = function (decoration) {
        ...
        ycoa.ajaxLoadPost("/api/second_sale/decoration.php", data, function (result) {
            if (result.code == 0) {
                ycoa.UI.toast.success(result.msg);
                reLoadData({action: 1});
            } else {
...
$(function () {
    ...
    $("#add_fillarrears_form #qq").keypress(function (e) {
        if (e.keyCode == 13) {
            if ($(this).val()) {
                $.get(ycoa.getNoCacheUrl("/api/second_sale/decoration.php"), {action: 5, qq: $(this).val()}, function (res) {
                    if (res.length == 1) {
                        var data = res[0];
                        $("#add_fillarrears_form input").each(function () {
                            var el = $(this);
    ...
    $("#btn_toexcel_primary").click(function () {
        ...
        if (start_time || end_time) {
            window.location.href = "/api/second_sale/decoration.php?action=10&start_time=" + start_time + "&end_time=" + end_time;
        }
    });
