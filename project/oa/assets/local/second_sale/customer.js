var CustomerListViewModel = new function () {
    ...
    self_.listCustomer = function (data) {
        ycoa.ajaxLoadGet("/api/second_sale/customer.php", data, function (results) {
            self_.customerList.removeAll();
            $.each(results.list, function (index, customer) {
                customer.dele = ycoa.SESSION.PERMIT.hasPagePermitButton("3050502");
                customer.edit = ycoa.SESSION.PERMIT.hasPagePermitButton("3050503");
                customer.begin = ycoa.SESSION.PERMIT.hasPagePermitButton("3050506") && ((customer.toplimit > customer.finish) && customer.status == 0);
                customer.end = ycoa.SESSION.PERMIT.hasPagePermitButton("3050506") && ((customer.toplimit > customer.finish) && customer.status != 0);
    ...
    self_.delCustomer = function (customer) {
        ycoa.UI.messageBox.confirm('确认删除？', function (del) {
            if (del) {
                customer.action = 3;
                ycoa.ajaxLoadPost("/api/second_sale/customer.php", JSON.stringify(customer), function (result) {
                    if (result.code == 0) {
...
$(function () {
    ...
    $("body").on("focus", '#myModal #customer_text,#dataTable #customer_text', function (data) {
        var self = this;
    ...
    $("#batchDelete").click(function () {
        ...
        if (id_ary.length == 0) {
            ...
            $.ajax({
                ...
                url: '../../api/second_sale/customer.php',
    ...
    $("body").on("click", '#add_customer_form #username', function () {
    ...
    $("body").on("click", '.dept_submit_btn', function () {
        ...
        ycoa.ajaxLoadPost("/api/second_sale/customer.php", data, function (result) {
            if (result.code == 0) {
                ycoa.UI.toast.success(result.msg);
                reLoadData({action: 1});
            } else {
...
var array = {
    type: [{id: 2, text: '优程售后'}, {id: 1, text: '平台销售'}, {id: 5, text: '班主任'}, {id: 3, text: '服务客服'}, {id: 4, text: '装修客服'}, {id: 6, text: '活动客服'}]
};
