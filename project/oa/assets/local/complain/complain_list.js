var complainListViewModel = new function () {
    var self_ = this;
    self_.list = ko.observable("list");
    self_.complainList = ko.observableArray([]);
    self_.listCompain = function (data) {
        ycoa.ajaxLoadGet("/api/complain/complain_list.php", data, function (results) {

            self_.complainList.removeAll();
            $.each(results.list, function (index, complainRecord) {
                if (complainRecord.isDeal == "已处理")
                {
                    complainRecord.dele = ycoa.SESSION.PERMIT.hasPagePermitButton("2070101");
                    complainRecord.edit = false;
                    complainRecord.deal = false;
                    complainRecord.second = ycoa.SESSION.PERMIT.hasPagePermitButton("2070106");
                } 
                ...
                {
                    complainRecord.dele = ycoa.SESSION.PERMIT.hasPagePermitButton("2070101");
                    complainRecord.edit = ycoa.SESSION.PERMIT.hasPagePermitButton("2070102");
                    complainRecord.deal = ycoa.SESSION.PERMIT.hasPagePermitButton("2070103");
                    complainRecord.second = false;
                }

                self_.complainList.push(complainRecord);
            });
    ...
    self_.delComplain = function (complaintRecord) {
        ycoa.UI.messageBox.confirm('确认删除？', function (del) {
            if (del) {
                ...
                ycoa.ajaxLoadPost("/api/complain/complain_list.php", JSON.stringify(complaintRecord), function (result) {
                    debugger
                    if (result.code == 0) {
    ...
    self_.secondComplain = function (complaintRecord) {
        $("#secondId").val(complaintRecord.id);
    };
...
$(function () {
    ko.applyBindings(complainListViewModel, $("#dataTable")[0]);
    reLoadData({action: 1});
    ...
    $("body").on("click", "#btn_toexcel_primary", function () {
        ...
        if (start_time || end_time) {
            location.href = '/api/complain/complain_list.php?start_time=' + start_time + '&end_time=' + end_time + '&action=11';
        }
    });
...
function reLoadData(data) {
    data.action = 1;
    complainListViewModel.listCompain(data);
}
...
function forDealSubmit()
{
    ...
    data.action = 4;
    debugger
    ycoa.ajaxLoadPost("/api/complain/complain_list.php", JSON.stringify(data), function (result) {
        debugger
        if (result.code == 0) {
...
function forSecondSubmit()
{
    ...
    var data = $("#add_second_form").serializeJson();
    data.action = 5;
    ycoa.ajaxLoadPost("/api/complain/complain_list.php", JSON.stringify(data), function (result) {
        if (result.code == 0) {
            $("#btn_close_secondback").click();
            $("#add_second_form input").val("");
            ycoa.UI.toast.success(result.msg);
            reLoadData({action: 1});
        } else {
            ycoa.UI.toast.error(result.msg);
