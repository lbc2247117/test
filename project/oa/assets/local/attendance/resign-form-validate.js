$('#add_resign_form').validate({
    ...
    submitHandler: function (form) {
        var data = $("#add_resign_form").serializeJson();
        data.action = 1;
        data.retroType = 1;
        data = JSON.stringify(data);
        ycoa.ajaxLoadPost("/api/attendance/resign.php", data, function (result) {
            if (result.code == 0) {
...
$('#add_resign_dpt_form').validate({
    ...
    submitHandler: function (form) {
        var data = $("#add_resign_dpt_form").serializeJson();
        data.action = 1;
        data.retroType = 2;
        data = JSON.stringify(data);
        ycoa.ajaxLoadPost("/api/attendance/resign.php", data, function (result) {
            if (result.code == 0) {
                $("#myDptModal").modal('hide');
                reLoadData({action: 1});
                ycoa.UI.toast.success(result.msg);
            } else {
                ycoa.UI.toast.error(result.msg);
