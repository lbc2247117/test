    ...
    self_.listPenalty = function (data) {
        ycoa.ajaxLoadGet("/api/attendance/penalty.php", data, function (results) {
            ...
            $.each(results.list, function (index, penalty) {
                penalty.addtime = penalty.addtime.split(' ')[0];
                penalty.read = false;
                if (penalty.isRead == 0 && results.role_type == 0) {
                    penalty.read = true;
                }
                penalty.isRead = penalty.isRead === 0 ? "" : "是"
...
$(function () {
    ...
    $("body").on("mouseenter", "#dataTable .data_list_div", function () {
        ...
        setTimeout(function () {
            ...
            if (t2 > 0 && t1 - t2 >= 1) {
                var w_id = self_.parent().attr("val");
    ...
    $("body").on("mouseleave", ".workflowLog_detail", function () {
        $(this).hide();
    });
