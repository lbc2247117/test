...
var causeleaveListViewModel = new function () {
    ...
    self_.listCauseleave = function (data) {
        ycoa.ajaxLoadGet("/api/attendance/adjustRest.php", data, function (results) {
            ...
            $.each(results.list, function (index, causeleave) {
                ...
                causeleave.begin_date = causeleave.begindate.split(' ')[0];
                causeleave.end_date = causeleave.enddate.split(' ')[0];
    ...
    self_.okCauseleave = function (causeleave) {
        causeleave.action = 2;
        ycoa.ajaxLoadPost("/api/attendance/adjustRest.php", JSON.stringify(causeleave), function (result) {
            if (result.code == 0) {
...
//        }
    ...
    $("#add_adjustRest_form #type").change(function () {
        var val = $(this).val();
        if (val != 1 && val != 2) {
            $("#add_adjustRest_form #salary_for_type").show();
            $("#add_adjustRest_form #salary").removeAttr("disabled");
        } else {
            $("#add_adjustRest_form #salary_for_type").hide();
            $("#add_adjustRest_form #salary").attr("disabled", "");
        }
    });
...
//    });
    ...
    $("body").on('click', '#btn_approve_primary', function () {
        if ($("input[type='radio'][name='isArgee']").is(':checked')) {
            ...
            object.action = 10;
            ycoa.ajaxLoadPost("/api/attendance/adjustRest.php", JSON.stringify(object), function (result) {
                if (result.code == 0) {
                    ycoa.UI.toast.success("操作成功~");
                    reLoadData({action: 1});
                } else {
...
function addInput(el) {
    var html = "<div class='form-group'><label class='control-label col-md-2' for='rest_date'>原休息日期<span style='color: red;'>*</span></label><div class='col-md-3'>";
    html += "<input class='form-control' onfocus='WdatePicker({dateFmt: \"yyyy-MM-dd\"})' type='text' name='rest_date' id='rest_date' placeholder='原休息日期'/>";
    html += "</div><label class='control-label col-md-2' for='adjust_to'>调休至<span style='color: red;'>*</span></label><div class='col-md-3'>";
    html += "<input class='form-control' onfocus='WdatePicker({dateFmt: \"yyyy-MM-dd\"})' type='text' name='adjust_to' id='adjust_to' placeholder='调休至'/>";
