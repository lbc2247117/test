...
var causeleaveListViewModel = new function () {
    ...
    self_.listCauseleave = function (data) {
        ycoa.ajaxLoadGet("/api/attendance/welfareleave.php", data, function (results) {
    ...
    self_.okCauseleave = function (causeleave) {
        causeleave.action = 2;
        ycoa.ajaxLoadPost("/api/attendance/welfareleave.php", JSON.stringify(causeleave), function (result) {
            if (result.code == 0) {
...
$(function () {
    ...
    $("#dataTable").searchAutoStatus([
        {id: 2, text: '病假'}, {id: 3, text: '产假'}, {id: 4, text: '丧假'}, {id: 5, text: '婚假'}, {id: 6, text: '陪产假'}, {id: 0, text: '全部'}
    ], function (d) {
...
//        }
    ...
    $("body").on("click", '#btn_toexcel_primary', function () {
        ...
        if (start_time || end_time) {
            location.href = '/api/attendance/welfareleave.php?start_time=' + start_time + '&end_time=' + end_time + '&action=11';
        }
    });
    ...
    $("body").on('click', '#btn_approve_primary', function () {
        if ($("input[type='radio'][name='isArgee']").is(':checked')) {
            ...
            object.action = 10;
            ycoa.ajaxLoadPost("/api/attendance/welfareleave.php", JSON.stringify(object), function (result) {
                if (result.code == 0) {
                    ycoa.UI.toast.success("操作成功~");
                    reLoadData({action: 1});
                } else {
...
function addInput(el) {
    ...
    html += "<input class='form-control' onfocus='WdatePicker({dateFmt: \"yyyy-MM-dd\"})' type='text' readonly='' name='starttime' id='starttime' placeholder='日期'/>";
    html += "</div><label class='control-label col-md-2' for='endtime'>结束时间<span style='color: red;'>*</span></label><div class='col-md-3'>";
    html += "<input class='form-control' onfocus='WdatePicker({dateFmt: \"yyyy-MM-dd\"})' type='text' readonly='' name='endtime' id='endtime' placeholder='日期'/>";
