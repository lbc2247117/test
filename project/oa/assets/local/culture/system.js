var SystemListViewModel = new function () {
    ...
    self_.downLoadFile = function (system) {
        window.location.href = system.file_path;
    };

     self_.downDoc = function(data)
    {
...
function forSysSubmit() {
    $("#add_system_form").ajaxSubmit({
        type: 'post',
        url: "/api/culture/system.php",
        success: function (data) {
                        if (data.code == 0) {
                $("#add_system_form").parent().parent().find('#btn_close_primary').click();
                ycoa.UI.toast.success(data.msg);
                SystemListViewModel.listSystem({});
            } else {
...
function forEditSysSubmit() {
    $("#edit_system_form").ajaxSubmit({
        type: 'post',
        url: "/api/culture/system.php",
        success: function (data) {
                        if (data.code == 0) {
