...
//        var formData = $("#add_manual_form").serializeJson();
//        add_manual(formData);
    }
});
function add_manual(data) {
    data.action = 1;
    data = JSON.stringify(data);
    ycoa.ajaxLoadPost("/api/culture/manual.php", data, function (result) {        if (result.code == 0) {
            $("#add_manual_form").parent().parent().find('#btn_close_primary').click();
            ycoa.UI.toast.success(result.msg);
            ManualListViewModel.listManual({action: 1});
        } else {
...
//        }
    ...
    submitHandler: function (form) {
//        var formData = $("#edit_manual_form").serializeJson();
//        edit_manual(formData);
    }
});
function edit_manual(data) {
    data.action = 2;
    data = JSON.stringify(data);
