...
var thirdpartnarListViewModel = new function () {
    ...
    self_.listthirdpartnar = function (data) {
        ycoa.ajaxLoadGet("/api/finance/thirdpartnar_checking.php", data, function (results) {
            ...
            $.each(results.list, function (index, thirdpartnar) {
                ...
                thirdpartnar.maintime = ycoa.format(thirdpartnar.maintime);
                ...
                } else if (thirdpartnar.st_is_approve == 0) {
                    thirdpartnar.edit = 1;
                } else {
                    thirdpartnar.dele = 1;
    ...
    self_.delthirdpartnar = function (thirdpartnar) {
        ...
        ycoa.UI.messageBox.confirm('确认财务审核吗？', function (btn) {
            if (btn) {
                ...
                ycoa.ajaxLoadPost("/api/finance/thirdpartnar_checking.php", JSON.stringify(obj), function (result) {
                    if (result.code == 0) {
...
$(function () {
    ...
    $("#btn_toexcel_primary").click(function () {
        ...
        if (start_time || end_time) {
            window.location.href = "/api/finance/thirdpartnar_checking.php?action=10&start_time=" + start_time + "&end_time=" + end_time;
        }
    });
