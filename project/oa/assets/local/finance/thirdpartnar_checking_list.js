...
var ThirdPartnarListModel = new function () {
    ...
    self_.listPartnar = function (data) {
        ycoa.ajaxLoadGet("/api/finance/thirdpartnar_checking_list.php", data, function (results) {
            ...
            if (results != null || results != "") {
                $.each(results.list, function (index, partnar) {
                    ...
                    partnar.transfer_time = (partnar.transfer_time).split(' ')[0];
                    partnar.compare_time = (partnar.compare_time).split(' ')[0];
                    partnar.is_question = partnar.is_question === '0' ? '无' : "有";
                    partnar.department = "售前";
...
$(function () {
    ...
    $("#btn_toexcel_primary").click(function () {
        ...
        if (start_time || end_time) {
            window.location.href = "/api/finance/thirdpartnar_checking_list.php?action=10&start_time=" + start_time + "&end_time=" + end_time;
        }
    });
});
