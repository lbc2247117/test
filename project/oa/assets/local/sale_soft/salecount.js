...
var SalecountListViewModel = new function () {
    ...
    self_.listSalecount = function (data) {
        ycoa.ajaxLoadGet("/api/sale_soft/salecount.php", data, function (results) {
            ...
            $.each(results.list, function (index, salecount) {
                ...
                salecount.reSetSalecount = ycoa.SESSION.PERMIT.hasPagePermitButton("3030306") && salecount.customer_id === 0;
                ...
                salecount.setcustomer = ycoa.SESSION.PERMIT.hasPagePermitButton("3030308");
    ...
    self_.setCustomer = function (salecount) {
        var html = "<div class='cu_div' id='doback_customer_" + salecount.id + "'>";
        html += "<select name='nick_name' id='nick_name' style='width:300px;height:41px; ;float:left' class='form-control'>";
        $.each(customer_list, function (index, d) {
            html += "<option value='" + d.id + "'>" + (d.text) + "</option>";
        });
        html += "</select>";
        html += "<span class='input-group-addon' id='setCustomerOk' isQQTeach = '" + (salecount.isQQTeach) + "' isTimely='" + (salecount.isTimely) + "' val='" + salecount.id + "' status='" + salecount.status + "' customer_id='" + salecount.customer_id + "'><i class='glyphicon glyphicon-ok' title='提交'></i></span>";
        html += "<span class='input-group-addon' id='setCancel' val='" + salecount.id + "' status='" + salecount.status + "'><i class='glyphicon glyphicon-remove' title='取消'></i></span>";
        html += "</div>";
        $("#customer_td_" + salecount.id).append(html);
        $(".doback_open").animate({opacity: 'toggle', width: '0px'}, 500, function () {
        ...
        $("#doback_customer_" + salecount.id).show();
        $("#doback_customer_" + salecount.id).animate({width: '383px', opacity: 'show'}, 500, function () {
            $(this).addClass("doback_open");
        });
    };
...
$(function () {
    ...
    $("#btn_toexcel_primary").live("click", function () {
        ...
        if (start_time || end_time) {
            location.href = '/api/sale_soft/salecount.php?start_time=' + start_time + '&end_time=' + end_time + '&action=11';
        }
    });
...
//    $("#add_refund_form #cd_alipay").autoRadio(array["cd_alipay"]);
    ...
    $("#setCustomerOk").live("click", function () {
        ...
        var v = $("#doback_customer_" + id).find("select").val();
        var t = $("#doback_customer_" + id).find("select").find('option:selected').text();
        t = t.replace(")", "").split("(");
        var nick_name = t[1];
        var customer = t[0];
        updateCL({id: id, status: status, customer_id: customer_id, customer_id2: v, nick_name2: nick_name, customer2: customer, mode: 'set', isQQTeach: $(this).attr('isQQTeach'), isTimely: $(this).attr('isTimely')});
...
function createTodayTotals() {
    $.get(ycoa.getNoCacheUrl("/api/sale_soft/salecount.php"), {action: 4}, function (results) {
        ...
        var TodayTotalsHtml = "<span style='color:black;'>百度:" + TodayBDTotals + "单(及时:" + TodayBDTimelyTotals + "单)&nbsp;</span>";
        TodayTotalsHtml += "<span style='color:green;'>360:" + Today360Totals + "单(及时:" + Today360TimelyTotals + "单)&nbsp;</span>";
        TodayTotalsHtml += "<span style='color:#EAEA1E;'>搜狗:" + TodaySGTotals + "单(及时:" + TodaySGTimelyTotals + "单)&nbsp;</span>";
        TodayTotalsHtml += "<span style='color:orange;'>UC:" + TodayUCTotals + "单(及时:" + TodayUCTimelyTotals + "单)&nbsp;</span>";
        TodayTotalsHtml += "<span style='color:brown;'>旺旺:" + TodayWWTotals + "单(及时:" + TodayWWTimelyTotals + "单)&nbsp;</span>";
        TodayTotalsHtml += "<span style='color:blue;'>优化站:" + TodayYHZTotals + "单(及时:" + TodayYHZTimelyTotals + "单)&nbsp;</span>";
        TodayTotalsHtml += "<span style='color: red;'>共:" + (TodayWWTotals + TodayBDTotals + Today360Totals + TodaySGTotals + TodayUCTotals + TodayYHZTotals) + "单";
        TodayTotalsHtml += "(及时:" + (TodayWWTimelyTotals + TodayBDTimelyTotals + Today360TimelyTotals + TodaySGTimelyTotals + TodaySGTimelyTotals + TodayYHZTimelyTotals) + "单)</span>";
        if (!isNaN(TodayWWTotals) && !isNaN(TodayBDTotals) && !isNaN(Today360Totals) && !isNaN(TodaySGTotals) && !isNaN(TodayUCTotals) && !isNaN(TodayYHZTotals)) {
...
var array = {
    ww: [{id: '无', text: '无', default: true}, {id: 'QQ', text: 'QQ'}],
    ...
    payment: [{id: '工行', text: '工行'}, {id: '农行', text: '农行'}, {id: '建行', text: '建行'}, {id: '中行', text: '中行'}, {id: '邮政', text: '邮政'}, {id: '信用社', text: '信用社'}, {id: '支付宝', text: '支付宝'}, {id: '财付通', text: '财付通'}, {id: '微信', text: '微信'}, {id: '淘宝', text: '淘宝', default: true}, {id: 'QQ钱包', text: 'QQ钱包'}, {id: '拍拍', text: '拍拍'}, {id: '微店', text: '微店'}],
