...
var SaleStatisticsMouthViewModel = new function () {
    ...
    self_.listSaleStatisticsDay = function (data) {
        ycoa.ajaxLoadGet("/api/sale_soft/saleStatisticsMouth.php", data, function (results) {
            self_.saleStatisticsMouthList.removeAll();
            $.each(results.list, function (index, saleStatistics) {
                var addtime = saleStatistics.addtime.split("-");
                saleStatistics.addtime = addtime[0] + "-" + addtime[1];
...
$(function () {
    ...
    $("#btn_toexcel_primary").live("click", function () {
        ...
        if (start_time || end_time) {
            location.href = "/api/sale_soft/saleStatisticsMouth.php?channel=" + channel + "&start_time=" + start_time + "&end_time=" + end_time + "&action=11";
        }
    });
});
