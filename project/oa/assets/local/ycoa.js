 ...
 * 并给日期选择框赋值
 */
function initServerDate(selector) {
    $.ajax({
        url: '',
        type: 'get',
        async: false,
        success: function (data, status, xhr) {
            ...
            var date = new Date(str); //把Date转换成时间对象
...
//计算选中金额
function countMoney(number) {
    var total = 0;
    $("#dataTable tbody .checkbox").each(function () {
        if ($(this).prop("checked")) {
            total = total + Number($(this).parents("tr").find("td").eq(number).text());
        }
    });
    $("#paging-container .checkboxMoney").html("已选中金额: " + total.toFixed(2) + "元");
}
...
var ycoa = new function ($) {
    ...
    this.initPagingContainers = function (node, data, callback, pageCallback) {
        ...
        {
            pagingNumbersTemplate += "<span style='margin-left:10px;'>本页金额合计:" + totalmoney + "元</span>";
            if (nototal != 1) {
                pagingNumbersTemplate += "<span style='margin-left:10px;color:red' class='checkboxMoney'>已选中金额:0.00元</span>";
            }
        }
    ...
    this.ajaxLoadGet = function (url, data, success) {
        ...
        $.get(ycoa.getNoCacheUrl(url), data, function (result) {
            ycoa.UI.block.hide();
            if (result != undefined && result.code == '0x301') {
                localStorage.setItem("loginError", "用户身份过期,请重新登录~");
                if (__path__ != "/page/index.html") {
                    window.top.location.href = __host__ + "/page/login.html";
                } else {
            ...
            } else if (result != undefined && result.code == '0x600') {
    ...
    this.UI = new function () {
         ...
         * @returns {undefined}
         */
        uiSelf.employeeSeleter = function (options, callback) {
            var el = options.el;
            var offset = el.offset();
            var top = offset.top + el.outerHeight() - 1;
            var left = offset.left;
            ...
            $("body").append(html);
            var empHtml = "";
            $.each(options.array, function (index, empList) {
                empHtml += "<div class='emp_text' val='" + empList.id + "'>" + empList.text + "</div>";
            });
            empHtml += "<div class='last'></div>";
            $("#emplist_items .emp_text").unbind("click");
            $("#emplist_items", $("#" + mainId)).html(empHtml);
            $("#emplist_items .emp_text").bind("click", function () {
                callback({id: $(this).attr("val"), text: $(this).text()}, el);
                $("#" + mainId).hide(500);
            });
            $(document).bind("click", function (e) {
                var target = $(e.target);
    ...
    this.SESSION = new function () {
        ...
        self.PERMIT = new function () {
            ...
            perSelf.hasPagePermitButton = function (id, call) {
                ...
                if (call) {
                    return call($.inArray(id, permitButtons) < 0 ? false : true);
                }
                return $.inArray(id, permitButtons) < 0 ? false : true;
            };
...
(function ($) {
    ...
    $.fn.searchDateTime = function (callback, title) {
        ...
        if (portletTitle.hasClass("portlet-title")) {
            ...
            $("body").on("mouseover", '#searchDateTime', function () {
            ...
            $("body").on("click", '#searchByDateTime', function () {
    ...
    $.fn.searchDateTimeSlot = function (callback) {
        ...
        if (portletTitle.hasClass("portlet-title")) {
            this.each(function () {
                ...
                var html = "<div class='input-group auto-group searchDateTimeSlot' style='max-width: 480px !important;*max-width: 580px !important;'>";
                ...
                html += time_date;
                html += "</select>";
                html += "<span class='input-group-addon exchange'><i class='fa fa-exchange'></i></span>";
                html += "<input type='text' id='endDate' class='form-control' data-date-format='yyyy-mm-dd' readonly='' placeholder='结束日期' style='float:left;width:111px;*margin-left:5px;'>";
                ...
                $("body").on("mouseover", "#startDate", function () {
                ...
                $("body").on("mouseover", "#endDate", function () {
                    $(this).datepicker({autoclose: true}, function (d) {
                ...
                $("body").on("click", "#searchByDateTimeSlot", function () {
    ...
    $.fn.searchUserName = function (callback, title, name) {
        ...
        this.each(function () {
            ...
            if (portletTitle.hasClass("portlet-title")) {
                ...
                $("body").on('click', "#searchBy" + name, function () {
     ...
     * @param {type} callback
     */
    $.fn.searchDept = function (callback) {
        var portletTitle = $(this).parent(".portlet-body").prev('.portlet-title');
        if (portletTitle.hasClass("portlet-title")) {
            portletTitle.append("<div class='btn-group bootstrap-select bs-select form-control input-small' style='float:right;margin-left:5px;border:solid 1px #cecece;min-width:150px !important;'><button id='dept_selecter' type='button' class='btn dropdown-toggle selectpicker ' style='background:#ffffff;' data-toggle='dropdown'><span class='search_dept_title filter-option pull-left'>按照部门筛选</span><span class='caret'></span></button></div>");
            $("body").on('click', "#dept_selecter", function () {
...
//                        }
//                    }
                    html += "<li rel='0' class='selected' v='" + d.id + "' t='" + d.text + "' d='" + date + "' c='" + d.cv + "'>";
                ...
                $("body").on("click", "#autoEditSelecter_" + date + " li", function () {
                    self.val($(this).attr("t"));
                    if (callback) {
                        callback({id: $(this).attr("v"), text: $(this).attr("t"), cv: $(this).attr("c"), el: self});
                    }
                });
            });
        } else {
     ...
     * */
    $.fn.searchAutoStatus = function (array, callback, title) {
        var portletTitle = $(this).parent(".portlet-body").prev('.portlet-title');
        if (portletTitle.hasClass("portlet-title")) {
            if (typeof (array) == 'object') {
                this.each(function () {
                    ...
                    html += "</div>";
                    portletTitle.append(html);
                    $("body").on('click', "#dropdown_menu_auto_status_" + date + " li", function () {
    ...
    $.fn.reLoad = function (callback) {
        var portletTitle = $(this).parent(".portlet-body").prev('.portlet-title');
        if (portletTitle.hasClass("portlet-title")) {
            portletTitle.append("<div class='tools' style='float: right; ' time='0'><i class='icon-reload' style='margin-left: 5px;cursor: pointer;font-size: 25px' time='0'></i></div>");
...
//                        }
                ...
                $("body").on("click", "#auto_radio_ul_" + date + " li", function () {
                    if (checkMore) {
                        var array = new Array();
    ...
    $("#importFile").change(function () {
        ...
        var file1 = $("#importFile").val();
        var filetype = file1.substring(file1.lastIndexOf(".") + 1);
        if (filetype == "xls" || filetype == "xlsx") {
            if (file) {
                $("#uploadFileInfo .fileName").text(file.name);
                var fileSize = 0;
                ...
                $("#uploadFileInfo .fileSize").text(fileSize);
            }
            $("#fileMessage").fadeOut();
            $("#uploadFileInfo").fadeIn();
        } else {
            $("#fileMessage").fadeIn();
        }
    });
    $("#uploadFileInfo .file-preview a").click(function () {
        $("#importFile").val("");
        $("#uploadFileInfo .fileName").text("");
        $("#uploadFileInfo .fileSize").text("");
        $("#uploadFileInfo").fadeOut();
    });
    ...
    $("body").on("click", ".addDate.plus", function () {
        addInput($(this));
    });

    $("body").on("click", ".addDate.minus", function () {
        $(this).parent().remove();
    });
