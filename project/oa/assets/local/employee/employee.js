    ...
    self_.listEmployee = function (data) {
        ycoa.ajaxLoadGet("/api/user/employee.php", data, function (results) {
            ...
            $.each(results.list, function (index, employee) {
                employee.birthday = ycoa.format(employee.birthday);
                employee.join_time = ycoa.format(employee.join_time);
                if (employee.leave_time) {
                    employee.leave_time = ycoa.format(employee.leave_time);
                }
...
//                employee.join_days = Math.floor(((Date.parse(new Date()) - date.getTime()) / 3600000) / 24);
            ...
            ycoa.initPagingContainers($("#paging-container"), results, function (pageSize) {
                reLoadData({mouth: $('#mouth').val(), pageno: ycoa.SESSION.PAGE.getPageNo(), pagesize: pageSize, deptid: $('#deptid').val(), status: $("#status").val(), searchName: $("#searchUserName").val(), sort: ycoa.SESSION.SORT.getSort(), sortname: ycoa.SESSION.SORT.getSortName()});
            }, function (pageNo) {
                reLoadData({mouth: $('#mouth').val(), pageno: pageNo, pagesize: ycoa.SESSION.PAGE.getPageSize(), status: $("#status").val(), searchName: $("#searchUserName").val(), deptid: $('#deptid').val(), sort: ycoa.SESSION.SORT.getSort(), sortname: ycoa.SESSION.SORT.getSortName()});
            });
        });
    };
...
$(function () {
    $("#dataTable").sort(function (data) {
        reLoadData({sort: data.sort, sortname: data.sortname, pagesize: ycoa.SESSION.PAGE.getPageSize(), deptid: $('#deptid').val(), searchName: $("#searchUserName").val(), status: $("#status").val()});
    });
    ...
    $("#dataTable").searchUserName(function (name) {
        reLoadData({searchName: name, mouth: $('#mouth').val(), status: $('#status').val(), deptid: $('#deptid').val(), pagesize: ycoa.SESSION.PAGE.getPageSize()});
    });
    if (getAction() != 4) {
        $("#dataTable").searchAutoStatus([{id: 1, text: '正式'}, {id: 5, text: '兼职'}, {id: 2, text: '试用'}, {id: 6, text: '试岗'}, {id: 4, text: '停薪留职'}, {id: 0, text: '全部'}], function (d) {
            //reLoadData({status: d.id});
            $('#status').val(d.id);
        }, '员工状态');
    }
    ...
    $("body").on("click", '.dept_submit_btn', function () {
    ...
    $("body").on("click", '#role_text', function () {
    ...
    $("body").on("click", '#myModal #dept1_text,#dataTable #dept1_text', function () {
        var self_ = this;
    ...
    $("body").on("mouseenter", '.data_list_div', function () {
        var self_ = $(this);
