...
function str_length_range($str, $min_length = null, $max_length = null, $equals = true) {
    ...
    if (isset($min_length) && isset($max_length))
        $res = $str_len > $min_length && $str_len < $max_length;
    elseif (isset($min_length) && !isset($max_length))
        $res = $str_len > $min_length;
    elseif (!isset($min_length) && isset($max_length))
        $res = $str_len < $max_length;
    else
        return false;
...
function gbk_2_utf8($str) {
    if (mb_detect_encoding($str, "ASCII,GB2312,GBK,UTF-8") == "EUC-CN")
...
function is_qq($value) {
    return filter_var($value, FILTER_VALIDATE_REGEXP, array('options' => array('regexp' => '/^[1-9][0-9]{5,11}$/')));
}
...
function get_first_letter_group($str) {
    ...
    if ($asc == -7226)
        return 'U-Z';
    if ($asc >= -20319 && $asc <= -18527)
        return 'A-E';
    if ($asc >= -18526 && $asc <= -16475)
        return 'F-J';
    if ($asc >= -16474 && $asc <= -14915)
        return 'K-O';
    if ($asc >= -14914 && $asc <= -12839)
        return 'P-T';
    if ($asc >= -12838 && $asc <= -10247)
        return 'U-Z';
    return null;
}
...
function time_diff_unit($begin_time, $end_time, $unit = 'h') {
    ...
    time_2_timestamp($end_time);
    $timediff = $end_time - $begin_time;
    if (str_equals($unit, 'm'))
        $diff = ceil($timediff / 60); // 这里默认用ceil进1运算，如需要四舍五入，请用round
    elseif (str_equals($unit, 'h'))
        $diff = ceil($timediff / 3600);
    elseif (str_equals($unit, 'd'))
        $diff = ceil($timediff / 3600 / 24);
    return $diff;
}
...
function time_diff_unit_floor($begin_time, $end_time, $unit = 'm') {
    ...
    time_2_timestamp($end_time);
    $timediff = $end_time - $begin_time;
    if (str_equals($unit, 'i'))
        $diff = floor($timediff / 60);
    elseif (str_equals($unit, 'h'))
        $diff = floor($timediff / 3600);
    elseif (str_equals($unit, 'd'))
        $diff = floor($timediff / 3600 / 24);
    elseif (str_equals($unit, 'm'))
        $diff = floor($timediff / 3600 / 24 / 30);
    return $diff;
}
