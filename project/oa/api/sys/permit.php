...
//    $dept2_id = request_int('dept2_id');
    ...
    if ($action == 1) {
        ...
        if ($role_id > 0) {
            ...
            if (strlen($role->get_permit()) <= 0 || str_equals($role->get_permit(), ';')) {
                ...
                if (isset($permit[0]))
                    $menu_ids = array_merge($menu_ids, explode(',', $permit[0]));
                if (isset($permit[1]))
                    $ctrl_ids = array_merge($ctrl_ids, explode(',', $permit[1]));
            }else {
    ...
    if ($action == 3) {
        ...
        if (strlen($permits) > 0 && !str_equals($permits, ';')) {
            ...
            if (isset($result[0]))
                $menu_ids = array_merge($menu_ids, explode(',', $result[0]));
            if (isset($result[1]))
                $ctrl_ids = array_merge($ctrl_ids, explode(',', $result[1]));
        } else {
            $role_id = $user->get_role_id();
            $role = new M_Role($role_id);
            $result = $role->load($db, $role);
            if (!$result[0])
                die_error(USER_ERROR, '获取角色信息失败');
    ...
    if ($action == 1) {
        ...
        pdo_transaction($db, function($db) use($dept_id, $role_ids, $permit) {
            if ($dept_id > 0) {
                ...
                if (!$result[0])
                    throw new TransactionException(PDO_ERROR_CODE, '保存部门权限失败', $result);
            }
            if (isset($role_ids)) {
                ...
                if (!empty($role_ids)) {
                    ...
                    if (!$result[0])
                        throw new TransactionException(PDO_ERROR_CODE, '保存职位权限失败', $result);
                }
            }
        });
        $post = new postMsgClass();
        $post->msgtype = msgType::change_permissons;
        $post->content = '1';
        ...
        if ($result == 'True')
            ...
            die_error(USER_ERROR, "保存权限成功，发送广播失败~");
    }
        ...
        pdo_transaction($db, function($db) use($user_ids, $permit) {
            if (isset($user_ids)) {
                ...
                if (!empty($user_ids)) {
                    ...
                    if (!$result[0])
                        throw new TransactionException(PDO_ERROR_CODE, '保存用户权限失败', $result);
                }
            }
        });
        echo_msg('保存用户权限成功~');
