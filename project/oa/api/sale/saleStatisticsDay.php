...
execute_request(HttpRequestMethod::Get, function() use($action) {
    if ($action == 1) {
        ...
        $manager_ids = array(0, 102, 701, 702, 703, 704, 709, 710,);
        $employee = get_employees()[$userid];
        $role_id = $employee['role_id'];
        $is_manager = in_array($role_id, $manager_ids);
        $sql = "SELECT s.id,s.channel,u.salecount_lv lv,s.userid,s.username,s.into_count,s.accept_count,s.deal_count,s.timely_count,s.amount,s.addtime ";
        $sql .= "FROM p_salestatistics s ";
        ...
        if ($is_manager) {
            $month_count_sql = "SELECT IFNULL(SUM(ps.into_count),0) AS into_count,IFNULL(SUM(ps.accept_count),0) AS accept_count,IFNULL(SUM(ps.deal_count),0) AS deal_count,";
            $month_count_sql.="IFNULL(SUM(ps.timely_count),0) AS timely_count,IFNULL(SUM(ps.amount),0) AS amount,'" . (isset($searchTime) ? $searchTime : date("Y-m-d")) . "' AS addtime from P_SaleStatistics ps WHERE 1=1 ";
            if (isset($searchTime)) {
    ...
    if ($action == 11) {
        ...
        $sql = "select s.id,s.channel,u.salecount_lv as lv,s.userid,s.username,s.into_count,s.accept_count,s.deal_count,s.timely_count,s.amount,s.addtime FROM P_SaleStatistics s ";
        $sql .="INNER JOIN m_user u ON s.userid = u.userid WHERE 1=1 ";
        if (isset($startTime)) {
        ...
        $field = array('username', 'lv', 'channel', 'into_count', 'accept_count', 'deal_count', 'timely_count', 'amount', 'commission', 'loss_number', 'loss_rate', 'timely_rate', 'timely_turnover_ratio', 'conversion_rate', 'average_price', 'addtime');
