...
execute_request(HttpRequestMethod::Get, function() use($action) {
    ...
    $sql = "SELECT pc.id,pc.userid,pc.username,pc.nickname,pc.toplimit,IFNULL(ps.finish,0) AS finish,pc.`status`,pc.qqReception,pc.tmallReception_qj,pc.tmallReception_zy,pc.starttime,pc.endtime FROM P_Customerrecord pc ";
    $sql .= "LEFT JOIN (SELECT ps.id,ps.customer,ps.customer_id, COUNT(ps.customer_id) AS finish FROM P_Salecount ps WHERE " . getWhereSql("ps") . " AND ps.customer_id != 0 GROUP BY ps.customer_id ) AS ps ON pc.userid = ps.customer_id ";
    if (isset($searchName)) {
