...
execute_request(HttpRequestMethod::Get, function() use($action) {
    if ($action == 1) {
        ...
        if (isset($searchName)) {
            $complaint_record->set_custom_where(" AND (mobile like '%" . $searchName . "%' OR complaint_custom like '%" . $searchName . "%' OR hand_personnel like '%" . $searchName . "%' OR qq like '%" . $searchName . "%' OR phone like '%" . $searchName . "%' )");
        }
    ...
    if ($action == 11) {
        ...
        $field = array('addtime', 'mobile', 'qq', 'phone', 'complaint_custom', 'complaint_content', 'hand_personnel', 'hand_result');
        ...
        $title_array = array('添加时间', '客户手机', '客户QQ', '来电号码', '投诉客服', '投诉内容', '处理人员', '处理结果');
...
execute_request(HttpRequestMethod::Post, function() use($action) {
    ...
    if ($action == 1) {
        ...
        $postmsg->msgtype = msgType::tousu;
        $postmsg->clientinfo = $complaint_record->get_qq();
        $postmsg->receiveruser = $complaint_record->get_complaint_custom();
        $msg = json_encode($postmsg);
        $result = curl_http_post(PUSH_MESSAGE_URL, $msg);
        if ($result!='True')
            die_error(USER_ERROR, '添加投诉记录成功,推送消息失败~');
