...
execute_request(HttpRequestMethod::Get, function() use($action) {
    if ($action == 1) {
        ...
        $sql = "SELECT s.userid,s.username,s.channel,u.salecount_lv lv,SUM(s.into_count) AS into_count,SUM(s.accept_count) AS accept_count,SUM(s.deal_count) AS deal_count,SUM(S.elderly_deal_count) AS elderly_deal_count,";
        $sql.="SUM(s.timely_count) AS timely_count,SUM(s.amount) AS amount,DATE_FORMAT(s.addtime,'%Y-%m') AS addtime FROM p_salestatistics_soft s ";
        ...
        $month_count_sql = "SELECT IFNULL(SUM(ps.into_count),0) AS into_count,IFNULL(SUM(ps.accept_count),0) AS accept_count,IFNULL(SUM(ps.deal_count),0) AS deal_count,IFNULL(SUM(ps.elderly_deal_count),0) AS elderly_deal_count,IFNULL(SUM(ps.timely_count),0) AS timely_count,IFNULL(SUM(ps.amount),0) AS amount,IFNULL(DATE_FORMAT(ps.addtime,'%Y-%m'),'" . $searchTime . "') AS addtime from P_SaleStatistics_soft ps ";
    ...
    if ($action == 11) {
        ...
        $sql = "SELECT s.id,s.channel,u.salecount_lv as lv,s.userid,s.username,SUM(s.into_count) AS into_count,SUM(s.accept_count) AS accept_count,SUM(s.deal_count) AS deal_count,SUM(s.elderly_deal_count) AS elderly_deal_count,SUM(s.timely_count) AS timely_count,SUM(s.amount) AS amount,DATE_FORMAT(s.addtime, '%Y-%m') AS addtime FROM P_SaleStatistics_soft s ";
        $sql .= "INNER JOIN m_user u ON s.userid = u.userid WHERE 1=1 ";
        if (isset($startTime)) {
        ...
        $field = array('username', 'channel', 'into_count', 'accept_count', 'deal_count', 'elderly_deal_count', 'timely_count', 'amount', 'commission', 'loss_number', 'loss_rate', 'timely_rate', 'timely_turnover_ratio', 'conversion_rate', 'average_price', 'addtime');
        $export->set_field($field);
        $export->set_field_width(array(12, 15, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 17));
