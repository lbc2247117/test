...
execute_request(HttpRequestMethod::Post, function() {
    ...
    if (str_equals($attdata, $z)) {
        if (isset($sqldata)) {
            $delete_sql = "DELETE FROM w_checklist WHERE DATE_FORMAT(checktime,'%Y-%m-%d') = DATE_FORMAT(NOW(),'%Y-%m-%d');";
            array_push($sql_array, $delete_sql);
            ...
            foreach ($array_2 as $k => $v) {
                ...
                if ($k == 0) {
                    $create_sql.= "CREATE TEMPORARY TABLE IF NOT EXISTS tmp_atuser ENGINE = MEMORY select " . $data_array_2[0] . " atid," . $data_array_2[1] . " bid,'" . $data_array_2[2] . "' aname ";
                } else {
