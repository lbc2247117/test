...
execute_request(HttpRequestMethod::Get, function() use($action) {
    ...
    if ($action == 2) {
        ...
        $customer->set_query_fields(array('userid', 'username', 'nickname','pay_rate'));
        $db = create_pdo();
...
execute_request(HttpRequestMethod::Post, function() use($action) {
    ...
    if ($action == 1) {
        ...
        pdo_transaction($db, function($db) use($customer) {
            ...
            if (!$result[0])
                throw new TransactionException(PDO_ERROR_CODE, '保存售后资料失败。' . $result['detail_cn'], $result);
        });
        echo_msg('添加成功');
    }
    //修改销售售后信息
    ...
    if ($action == 4) {
        ...
        pdo_transaction($db, function($db) use($sql) {
            ...
            if (!$result[0])
                throw new TransactionException(PDO_ERROR_CODE, '启动失败~' . $result['detail_cn'], $result);
        });
        echo_msg('启用成功');
    }
    //批量删除
    if ($action == 10) {
        $id = $_POST['id'];
        $sql = "delete from p_customerrecord_second where id in($id)";
        $db = create_pdo();
        $result = $db->exec($sql);
        if (!$result)
    ...
    if ($action == 41) {
        ...
        pdo_transaction($db, function($db) use($sql) {
            $result = Model::execute_custom_sql($db, $sql);
            if (!$result[0])
                throw new TransactionException(PDO_ERROR_CODE, '暂停失败~' . $result['detail_cn'], $result);
        });
        echo_msg('启用成功');
    }
});
