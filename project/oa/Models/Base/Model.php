...
class Model {
    ...
    private static function internal_save_model(\PDO $db, Model $model, $sql, $operation_fields, $ignore_affected_row_count = false) {
        ...
        if (!$stat->execute()) 
            return self::create_statement_error("保存实体{$model->class}时,执行失败", $stat);
    ...
    public static function model_count(\PDO $db, Model $model) {
        $model4count = $model;
        $model4count->set_query_fields('COUNT(*) AS total_record_count');
        $model4count->sql_builder->_order_by = array();
        $model4count->set_limit_count(NULL);
        $result = Model::execute_scalar($db, NULL, $model4count);
     ...
     * @deprecated 尚未实现
     */
    public static function model_sum(\PDO $db, Model $model) {
        return 0;
    }
     ...
     * @deprecated 尚未实现
     */
    public static function model_avg(\PDO $db, Model $model) {
        return 0;
    }
     ...
     * @deprecated 尚未实现
     */
    public static function model_max(\PDO $db, Model $model) {
        return 0;
    }
     ...
     * @deprecated 尚未实现
     */
    public static function model_min(\PDO $db, Model $model) {
        return 0;
    }
     ...
     * @deprecated 尚未实现
     */
    public function sum(\PDO $db) {
        return self::model_sum($db, $this);
    }

    /**
     * @deprecated 尚未实现
     */
    public function avg(\PDO $dbL) {
        return self::model_avg($db, $this);
    }

    /**
     * @deprecated 尚未实现
     */
    public function max(\PDO $db) {
        return self::model_max($db, $this);
    }

    /**
     * @deprecated 尚未实现
     */
    public function min(\PDO $db) {
        return self::model_min($db, $this);
    }
