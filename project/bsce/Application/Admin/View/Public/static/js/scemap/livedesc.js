var
        GET_TYPE_API = '../Allsce/getLivetag',
        EDIT_API = '../Allsce/updateLive',
        CNT = new Vue({
            el: '#wrapper',
            data: {
                sceMapid: URL_PARAM('id'),
                videoTypes: [],
                id: '',
                audioUrl: '',
                staticAudioUrl: '',
            ...
            methods: {
                ...
                saveEdit: function () {
                    ...
                    if (STRING_LENGTH(TRIM(CNT.videoName)) > 32) {
                        BASE.showAlert('直播名称最多只能输入32个字符');
                        return;
                    }
                    if (STRING_LENGTH(TRIM(CNT.videoRemark)) > 240) {
                        BASE.showAlert('直播描述最多只能输入240个字符');
                        return;
                    }
                    ...
                    $('#editform').ajaxSubmit({
                        ...
                        success: function (rst) {
                            ...
                            $.post('../Allsce/getVideomsg', {id: CNT.id}, function (rstData) {
                ...
                cancle: function () {
                    window.location.href = 'live.html?id=' + CNT.sceMapid;
                },
                playMusic: function () {
